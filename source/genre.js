...
function captureGenre(doc) {
	...
	let en = { type: this.type, id: this.url.match(base.genreExp)[1], name: n, description: txt };
	en.songs = new Array();
	...
	for (let ele of eles) {
		...
		pool.songs.weigh(sid, { name: sng.textContent, weight: 8 });
		pool.artists.weigh(uid, { name: art.textContent, weight: 6 });
	}
	en.albums = new Array();
	...
	for (let ele of eles) {
		...
		pool.albums.weigh(aid, { name: alb.textContent, weight: 7 });
		pool.artists.weigh(uid, { name: art.textContent, weight: 5 });
	}
	...
	for (let ele of eles) {
		...
		pool.artists.weigh(uid, { name: art.textContent, weight: 8 });
	}
	if (this.type == "genre") {
		en.styles = new Array();
		...
		for (let i = eles.length - 1; i >= 0; i--) {
			...
			queue.unshift({ mode: "frame", type: "style", url: an.href, callback: captureGenre, data: en.styles });
		}
	}
	this.data.push(en);
}
